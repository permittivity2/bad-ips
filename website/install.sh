...
# Setup nftables infrastructure using installer script
setup_nftables_infrastructure() {
    echo "Setting up nftables infrastructure..."

    # The installer script is already installed by this point
    if [ -f /usr/local/sbin/bad_ips_installer.sh ]; then
        chmod +x /usr/local/sbin/bad_ips_installer.sh

        if /usr/local/sbin/bad_ips_installer.sh; then
            echo -e "${GREEN}✓${NC} nftables infrastructure created"
        else
            echo -e "${RED}✗${NC} Failed to create nftables infrastructure"
            exit 1
        fi
    else
        echo -e "${RED}✗${NC} bad_ips_installer.sh not found"
        exit 1
    fi
}
